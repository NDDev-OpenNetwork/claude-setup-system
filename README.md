...
Installs, reselects, restores and removes a complete Claude Code harness configuration in a caller-named target directory.
...
claude-setup-system select minimal      --target ~/.tool-config
...
claude-setup-system restore --backup slot-000000000002 --target ~/.tool-config
...
Point `CLAUDE_SETUP_SYSTEM_SETUP_CATALOG` at a directory to use setups of your own.
...
| Documented configuration home | `~/.claude` |
...
```bash
CLAUDE_INSTALL_DIR=/opt/claude-setup-system sh install.sh
```

The same variable on both scripts, and it is `CLAUDE_INSTALL_DIR`
...
```bash
ai-stp provider conformance --harness claude-code \
  --executable ~/.local/bin/claude-setup-system \
...
docker run --rm -v "$HOME/.config:/config" \
  ghcr.io/nddev-opennetwork/claude-setup-system:0.0.7 \
