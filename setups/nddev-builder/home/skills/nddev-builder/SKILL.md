---
name: nddev-builder
description: Build, review or validate a Claude Code setup for claude-setup-system -- its owned surfaces, the components it carries, the lifecycle it performs, and the checks it must pass. Use when changing claude-setup-system or the native artifacts a setup writes.
---

# NDDev Builder

The entry point for work on `claude-setup-system`. Keep changes
...
2. **Prefer what the program answers over a copy of it.** Ask the binary:
   ...
   `crates/claude-setup-system/src/main.rs` and the baseline a test binds it to.
