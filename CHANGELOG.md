...
The Claude Code provider retains the existing protocol and preservation contract.
...
The Claude Code provider retains prior setup identity and written ownership.
...
declares.** The rule a model reads while working on Claude Code said the product
...
**A walk written when a surface held one kind.** Claude Code's table gained a
...
**Claude Code's table gained a kind.** Where a product loads a plugin from a
...
**Claude Code declares the `instruction` kind**, and the surface that routes it is
...
Claude Code's provider publishes a projection profile per *scope* -- a second
...
Starts Claude Code, and takes over a target the estate before this one
still claims.
...
Installs Claude Code itself, not only its configuration.
...
Claude Code harness configuration in a caller-named target directory.
